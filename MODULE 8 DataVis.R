...
histObj_freq <- hist(iris$Sepal.Length, freq=TRUE, col="blue")  # Draw vis and store object

# Probability density plot, use prob=TRUE. breaks arg controls # of bins

# What it does is set the vertical axis so that if we were to add the areas 
# of all the rectangles, we would find that they sum to the number 1.
# Calculate the area of each rectangle, add them up to equal 1
...
# Using pData, barplot() shows number of observations in each class ==> CIT has values 1 .. 5
...
# To fully understand density, you'll need to understand kernal functions, 
# probability density functions (PDF), and probability density estimation - all
# mathematical concepts in statistics. 

# Many believe that kernal density plots are a much more effective way to view 
...
# Scatterplots - the most used type of plot for exploration. Use to visualize
...
# Quantile-quantitle plot: plots the quantiles of one variable against another
# axes represent the 1st percentile up to the 100th precentile of the variables

qqplot(x,y)
abline(c(0,1))

# NOTE: if the two distributions were exactly the same, the percentiles would land
# on the line (intercept of 0 and slope of 1)

# 1. Normal Q-Q plots can be extremely effective in highlighting glaring outliers 
#    in a data sequence. 
# 2. An informal graphical test for showing a data sequence is normally distributed
#    i.e. approximated by a straight line
# 3. ALSO useful in highlighting: distributional asymmetry, heavy tails, outliers,
#    multi-modality, and other data anomalies. 
