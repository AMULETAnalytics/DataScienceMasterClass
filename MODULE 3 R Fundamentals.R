...
# What's the difference between logical operators & and &&, | and || ??

# No surprises below, & and && work the same
1==1
#[1] TRUE
...
1:3==1:3 && 1:3==1:3       # Evaluate to TRUE since TRUE AND TRUE is TRUE
#[1] TRUE

1:3==1:3 && 1:3==c(1,3,3)  # Same as above since only 1st elements
[1] TRUE
...
# R has pass-by-value semantics, which minimizes accidental side effects
# (a good thing). However, when code is organized into many functions/methods
# for reusability/readability/maintainability and when that code needs to
# manipulate large data structures through, e.g., big data frames, through 
# a series of transformations/operations the pass-by-value semantics leads 
# to a lot of copying of data around and much heap thrashing (a bad thing). 
# For example, a 50MB data frame that is passed as a function argument is
# not managed well. 
...
# Define a "anonymous" function inside of lappy() for extracting first column of each matrix
